...
Подробное [определение входных параметров](./components-props.md#Проверка-входных-параметров) имеет два преимущества:
...
`key` с `v-for` _всегда_ обязателен для компонентов, для поддержания внутреннего состояния компонента и его поддерева. Даже для элементов это хорошая практика для поддержания предсказуемого поведения, такого как [согласованности объекта](https://bost.ocks.org/mike/constancy/) в анимации.
...
По нашему опыту, лучше _всегда_ добавлять уникальный ключ, чтобы вам и вашей команде никогда не приходилось беспокоиться об этих крайних случаях. Затем в редких критически зависимых от производительности случаях, когда согласованность объекта не требуется — вы можете сделать сознательное исключение.
...
### Местные стили компонента <sup data-p="a">важно</sup>
...
### Именование закрытых свойств <sup data-p="a">важно</sup>
...
Что касается приставки `$`, то в рамках экосистемы Vue это специальные свойства экземпляра, которые публично доступны пользователю, поэтому использование его для _закрытых_ свойств было бы нецелесообразным.
...
**Имена файлов [однофайловых компонентов](./single-file-components.md) должны быть всегда в PascalCase или всегда в kebab-case.**
...
* Поскольку имена компонентов всегда должны состоять из нескольких слов, это соглашение запрещает вам выбирать произвольную приставку для простых компонентов-обёрток (например, `MyButton`, `VueButton`).
...
* Перестроение кода становится более сложным, потому что поиска с заменой часто будет недостаточно, чтобы обновить относительные ссылки на перемещённый компонент.
...
**Компоненты без содержимого должны быть самозакрывающимися тегами в [однофайловых компонентах](./single-file-components.md), строковых шаблонах и [JSX](./render-function.md#JSX) — но никогда в DOM-шаблонах.**
...
**В большинстве проектов имена компонентов всегда должны быть в PascalCase в [однофайловых компонентах](./single-file-components.md) и строковых шаблонах — но в kebab-case в случае DOM-шаблонов.**
...
* `<MyComponent>` более зрительно отличается от элемента HTML из одного слова, нежели `<my-component>`, потому что есть две заметных разницы в символах (две заглавных), а не только одна (тире).
...
**Стиль именования компонентов в JS/[JSX](./render-function.md#JSX) всегда должен быть PascalCase, хотя они могут быть в kebab-case внутри строк для простых приложений, которые используют только глобальную регистрацию компонентов через `Vue.component`.**
...
**Входные параметры должны всегда использовать camelCase при определении, но kebab-case в шаблонах и [JSX](./render-function.md#JSX).**
...
В JavaScript написание объектов с несколькими свойствами в несколько строк считается хорошей практикой, потому что при таком написании её гораздо легче читать. Наши шаблоны и [JSX](./render-function.md#JSX) стоит рассматривать также.
...
### Порядок настроек компонента/экземпляра <sup data-p="c">рекомендуется</sup>

**Опции компонента/экземпляра должны быть упорядочены согласованно.**
...
6.  **Композиция** (объединение свойств в настройках)
...
8.  **Местное состояние** (местные реактивные свойства)

  * `data`
  * `computed`

9.  **События** (обратновызовы вызываемые реактивными событиями)

  * `watch`
  * События перехватчиков жизненного цикла (в порядке их вызова)
      * `beforeCreate`
      * `created`
      * `beforeMount`
      * `mounted`
...
4.  **Изменители отрисовки** (изменяют способ отрисовки элемента)
...
### Пустые строки между настроек компонента/экземпляра <sup data-p="c">рекомендуется</sup>
...
**[Однофайловые компоненты](./single-file-components.md) должны всегда использовать один порядок для корневых тегов секций `<script>`, `<template>` и `<style>`, заканчиваясь `<style>`, потому что всегда требуется хотя бы одна из двух других.**
...
### Отбиратели элементов при использовании `scoped` <sup data-p="d">используйте с осторожностью</sup>
...
Проблема в том, что большое количество [отбирателей атрибутов элементов](http://stevesouders.com/efws/css-selectors/csscreate.php?n=1000&sel=a%5Bhref%5D&body=background%3A+%23CFD&ne=1000) (например, `button[data-v-f3f3eg9]`) будет значительно медленнее [отбирателей классов](http://stevesouders.com/efws/css-selectors/csscreate.php?n=1000&sel=.class%5Bhref%5D&body=background%3A+%23CFD&ne=1000) (например, `.btn-close[data-v-f3f3eg9]`), поэтому отбиратели классов должны быть предпочтительными всегда, когда это возможно.
...
### Неявное общение между родительским и дочерними компонентами <sup data-p="d">используйте с осторожностью</sup>
