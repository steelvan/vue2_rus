...
- Уменьшает ошибки при реализации новой функциональности
- Улучшает дизайн кода
- Способствует перестроению кода
...
- перестроение дублирования логики (создание `wrapper` и установка переменной `username`) в фабричную функцию
...
Vue Test Utils и огромная экосистема JavaScript обеспечивают множество инструментов чтобы обеспечить почти 100% покрытия тестами. Однако модульные тесты являются лишь частью пирамиды тестирования. Есть и другие типы тестов, включая тесты e2e (end-to-end) и тесты снимками.
Модульные тесты — это самые маленькие и самые простые тесты — они проверяет утверждения на маленьких единицах работы, изолируя каждую часть единственного компонента.
...
Модульные тесты наиболее полезны во время разработки, либо чтобы помочь разработчику подумать спроектировать компонент, либо перестроить существующий компонент. Они запускаются каждый раз при изменении кода.
